...
def find_gridcell_area(center_lon, center_lat, resolution_lon, resolution_lat):
	...
	# Find gridcell edges
	lon_min = center_lon - resolution_lon/2.
	lon_max = center_lon + resolution_lon/2.
	lat_min = center_lat - resolution_lat/2.	
	lat_max = center_lat + resolution_lat/2.

	# Convert coordinates to radians
	lon_min = (360 + lon_min)*np.pi/180.
	lon_max = (360 + lon_max)*np.pi/180.
	lat_min = lat_min*np.pi/180.
	lat_max = lat_max*np.pi/180.
...
def find_cluster_area(coordinates, lons, lats, resolution_lon, resolution_lat):
	...
	for i in range(0, n):
		...
		# Calculate area
		area = find_gridcell_area(center_lon, center_lat, resolution_lon, resolution_lat)
...
def find_weighed_centroid(lats_array, lons_array, intensities, lons, lats):
	...
	npixels = len(lons_array)
        ...
        if npositive > 0 and nnegative > 0:
            ...
            if hist[0] > 0 and hist[-1] > 0:    # If it does, check if it's a polar cluster
                    ...
                    if len(np.where(hist==0)[0]) == 0:
                            ...
                            hist2d, lats_edges, lons_edges = np.histogram2d(lats_array, lons_array, bins=(lats, lons));
	    ...
	    centroid_lon = np.average(lons_array, weights=intensities)
            ...
            lons_array_temp[lons_array>lon_shift] = lons_array_temp[lons_array>lon_shift] - lon_shift
            lons_array_temp[lons_array<=lon_shift] = lons_array_temp[lons_array<=lon_shift] + 360 - lon_shift
...
def find_drought_clusters(data_matrix, lons, lats, resolution_lon, resolution_lat):
	...
	for i in range(0, npixels):
		...
		if seed_pixel not in clustered_pixels:
			...
			cluster_dictionary[cluster_count] = {'coordinates':[], 'area': [], 'intensity':[], 'centroid':[]}
			...
			new_additions = []; new_additions.append(seed_pixel)
					...
					if len(positive_matches) > 0:
						...
						for pixel in positive_matches:
							...
							if pixel not in clustered_pixels:
								...
								cluster_dictionary[cluster_count]['coordinates'].append(pixel)
			...
			current_cluster_coordinates = cluster_dictionary[cluster_count]['coordinates']

                        # Clean up in case there are any duplicates and resave
                        current_cluster_coordinates = list(set(current_cluster_coordinates))
                        cluster_dictionary[cluster_count]['coordinates'] = current_cluster_coordinates
			...
			centroid_lat, centroid_lon = find_weighed_centroid(lats_array, lons_array, intensities_array, lons, lats)
			...
			cluster_area = find_cluster_area(current_cluster_coordinates, lons, lats, resolution_lon, resolution_lat)
...
def cluster_in_Sahara(centroid_lon, centroid_lat):
	...
	if centroid_lat >= 20 and centroid_lat <= 25 and centroid_lon >= -17 and centroid_lon <= 34:
		return True
	else:
		return False
...
def filter_small_clusters(data_matrix, cluster_count, cluster_dictionary, area_threshold):
	...
	for i in range(1, cluster_count + 1):
		...
		if cluster_area >= area_threshold and not in_sahara:
			...
			filtered_cluster_dictionary[filtered_cluster_count] = {'coordinates':[], 'area':[], 'intensity':[], 'centroid':[]}
			filtered_cluster_dictionary[filtered_cluster_count]['coordinates'] = cluster_dictionary[i]['coordinates']
			filtered_cluster_dictionary[filtered_cluster_count]['area'] = cluster_dictionary[i]['area']
			filtered_cluster_dictionary[filtered_cluster_count]['intensity'] = cluster_dictionary[i]['intensity']
			filtered_cluster_dictionary[filtered_cluster_count]['variability'] = cluster_dictionary[i]['variability']
			filtered_cluster_dictionary[filtered_cluster_count]['centroid'] = cluster_dictionary[i]['centroid']
...
def load_drought_cluster_data(data_path, start_date, tsteps, nlons, nlats):
	...
	for i in range(0, tsteps):
		drought_clusters_dictionary[i] = {'clusters_characteristics': {}, 'cluster_count':[]}
	...
	for i in range(0, tsteps):
		...
		f_name_dictionary = data_path + 'cluster-dictionary_' + str(current_date) + '.pck'
...
def track_clusters_and_save(data_path, start_date, end_date, tsteps, lons, lats, drought_threshold, dataset):
	...
	f_name = data_path + 'tracked_clusters_dictionary_' + str(start_date.year) + '-' + str(end_date.year) + '.pck'
...
def track_clusters(drought_cluster_dictionary, drought_matrix, start_date, end_date):
	...
	for i in range(1, first_count+1):
		...
		cluster_data_dictionary[cluster_ID_count][t_idx] = {} # Each property is a function of time 
		cluster_data_dictionary[cluster_ID_count][t_idx]['area'] = first_dictionary[i]['area']
		cluster_data_dictionary[cluster_ID_count][t_idx]['intensity'] =  first_dictionary[i]['intensity']
		cluster_data_dictionary[cluster_ID_count][t_idx]['centroid'] =  first_dictionary[i]['centroid']
		cluster_data_dictionary[cluster_ID_count][t_idx]['coordinates'] =  first_dictionary[i]['coordinates']
	...
	for i in range(0, nsteps-1):
		...
		future_dictionary = drought_cluster_dictionary[i+1]['clusters_characteristics']
		...
		for k in range(1, future_count + 1):
			...
			future_to_current_graph[k]['local'] = []	# Keeps track of the numbering between two time steps
			future_to_current_graph[k]['global'] = []	# Keeps track of the running cluster count
		...
		for j in range(0, current_count):
			...
			current_cluster_coordinates = set(cluster_data_dictionary[cluster_ID][t_idx]['coordinates'])
			...
			for k in range(1, future_count+1):
				...
				overlap = len(current_cluster_coordinates & future_cluster_coordinates)/np.float(len(current_cluster_coordinates))
		...
		sorted_future = [x for (y,x) in sorted(zip(future_areas,copy_future),reverse=True)]
		...
		for k in sorted_future:
			...
			if nclusters == 0:
				...
				cluster_data_dictionary[cluster_ID_count][next_date]['area'] = future_dictionary[k]['area']
				cluster_data_dictionary[cluster_ID_count][next_date]['intensity'] =  future_dictionary[k]['intensity']
				cluster_data_dictionary[cluster_ID_count][next_date]['centroid'] =  future_dictionary[k]['centroid']	
				cluster_data_dictionary[cluster_ID_count][next_date]['coordinates'] =  future_dictionary[k]['coordinates']
				...
				largest_area = 0; largest_cluster = 0
				for j in range(0, nclusters):
					...
					if area > largest_area and (current_ID not in cluster_data_dictionary[next_date]):
				...
				if largest_cluster == 0:
					...
					for j in range(0, nclusters):
						...
						# Append geneology
						cluster_data_dictionary[clusters_to_link[j]]['parent_of'].append(cluster_ID_count)
						cluster_data_dictionary[cluster_ID_count]['child_of'].append(clusters_to_link[j])
					...
					cluster_data_dictionary[largest_cluster][next_date]['area'] = future_dictionary[k]['area']
					cluster_data_dictionary[largest_cluster][next_date]['intensity'] =  future_dictionary[k]['intensity']
					cluster_data_dictionary[largest_cluster][next_date]['centroid'] =  future_dictionary[k]['centroid']
					cluster_data_dictionary[largest_cluster][next_date]['coordinates'] =  future_dictionary[k]['coordinates']
					...
					if nclusters > 1:
						for j in range(0, nclusters):
							...
							cluster_data_dictionary[clusters_to_link[j]]['merging_events_by_date'].append(next_date)			
...
def find_clusters_displacements(centroid_lons, centroid_lats):
	...
	if tsteps > 1:
		...
		for i in range(0,tsteps-1):
			...
			displacements[i] = find_geo_distance(centroid_lons[i], centroid_lats[i], centroid_lons[i+1], centroid_lats[i+1])
		...
		end_points = find_geo_distance(centroid_lons[0], centroid_lats[0], centroid_lons[-1], centroid_lats[-1])
	else:
		displacements = [0]
...
def extract_tracks(cluster_data_dictionary):
	...
	for cluster_ID in range(1, nclusters + 1):
		...
		while date_temp <= cluster_end:
			...
			# Current centroid
			centroid_lon, centroid_lat = cluster_data_dictionary[cluster_ID][date_temp]['centroid']
			...
			tracks_dictionary[cluster_ID]['areas'].append(cluster_data_dictionary[cluster_ID][date_temp]['area'])
			tracks_dictionary[cluster_ID]['intensities'].append(cluster_data_dictionary[cluster_ID][date_temp]['intensity'])
				
			# Update date
			date_temp = date_temp + relativedelta(months=1)

		# Calculate centroid's displacements and speeds
		total_displacement, end_points, displacements = find_clusters_displacements(tracks_dictionary[cluster_ID]['lons'], tracks_dictionary[cluster_ID]['lats'])
